(ns brex.main
  "Kotodama WASM entrypoint for the Brex clean-room actor (L4) — Clojure port.
  ...
  facts (`brex.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"brex.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "brex-compat" :tier tier :entities entities} 200])
